...
async function chargerDonnees() {
  ...
  const { data: partenairesData, error: partenairesError } = await useApi(`${API_BASE_URL}/api/partenaires/`, {
    method: "GET",
    server: false,
    cacheTTL: 10 * 60 * 1000,
  });
...
const ajouterPartenaire = async () => {
  ...
  const { data, error } = await useApi(`${API_BASE_URL}/api/partenaires/`, {
    method: "POST",
    body: {
...
const enregistrerModifications = async () => {
  ...
  const { data, error } = await useApi(`${API_BASE_URL}/api/partenaires/${id}/`, {
